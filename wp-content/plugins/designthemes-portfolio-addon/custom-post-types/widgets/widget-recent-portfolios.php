<?php
class DTPortfolio_Widget extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			"classname"   => 'widget_popular_entries',
			'description' => esc_html__('To list out portfolio items', 'dtportfolio')
		);

		parent::__construct(false, esc_html__(' Portfolio','dtportfolio'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
        		<?php esc_html_e('Title:','dtportfolio');?> 
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','dtportfolio');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'dtportfolio');?></option><?php
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count')); ?>">
	    		<?php esc_html_e('No.of posts to show:','dtportfolio');?>
	    	</label>
	...
	function widget($args, $instance) {
		...
		$arg = array( 'posts_per_page' => $_post_count,
		...
		if( !empty( $_post_categories ) ) {
			...
			$arg = array( 'posts_per_page' => $_post_count,
				...
				'tax_query' => array( array(
					...
					'field'    => 'term_id',
					'operator' => 'IN',
					'terms'    => $_post_categories
				) )
			);
		}

		echo dtportfolio_before_after_widget( $before_widget ); 

		if( !empty( $title ) ) {

			echo dtportfolio_widget_title( $before_title . $title . $after_title );			
		}
		...
		echo "<div class='recent-portfolio-widget'><ul>";
			 ...
			 if($the_query->have_posts()) :
			 	...
			 	echo "<li>".esc_html__('No Portfolio Entries found','dtportfolio')."</li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";

		echo dtportfolio_before_after_widget( $after_widget ); 
	}
}
?>