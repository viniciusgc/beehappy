<?php 
class Owly_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			"classname"   => 'widget_flickr',
			'description' => esc_html__('A widget that show last flickr photo streams', 'owly')
		);

		parent::__construct( false, OWLY_THEME_NAME.esc_html__(' Flickr Widget','owly'), $widget_options );
	}
        ...
        <p>
         ...
         <p>
         	<label for="<?php echo esc_attr($this->get_field_id('size'));?>">
         		<?php esc_html_e('What pictures to display','owly'); ?>
         		<select class="widefat" id="<?php echo esc_attr($this->get_field_id('size')); ?>" name="<?php echo esc_attr($this->get_field_name('size')); ?>"><?php
         			$a = array(
						"s" => esc_html__("square",'owly'),
						"t" => esc_html__("thumbnail",'owly'),
						"m" => esc_html__("medium",'owly')
         			);
