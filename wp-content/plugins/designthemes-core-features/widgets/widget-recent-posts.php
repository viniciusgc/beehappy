<?php
class Owly_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'owly')
		);

		parent::__construct(false,OWLY_THEME_NAME.esc_html__(' Posts','owly'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','owly');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'owly');?></option><?php
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','owly');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'owly');?>
        </p>  

	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count'));?>">
	    		<?php esc_html_e('No.of posts to show:','owly');?>
	    	</label>
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : OWLY_THEME_URI . '/images/dummy-images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".owly_excerpt(10)."</div>";
					endif;
					
					echo '<div class="entry-meta">';
						echo '<p> <span class="fas fa-clock"> </span> '.get_the_date('M').' '.get_the_date('d').'</p>';
						$commtext = "";
						if((wp_count_comments($post->ID)->approved) == 0)	$commtext = '0';
						else $commtext = wp_count_comments($post->ID)->approved;
						echo '<p> <a href="'.get_permalink().'#comments"><span class="fas fa-comment"> </span> '.$commtext.'</a></p>';
					echo '</div>';
				echo "</li>";
			 endwhile;
			 else:
			 	echo "<li><h4>".esc_html__('No Posts found','owly')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";
